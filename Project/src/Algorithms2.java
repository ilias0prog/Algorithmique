...
public interface Algorithms2 {
     ...
     * pour s'assurer que la méthode est fiable dans tous les cas de figure
     *
     ...
     * @return : true if targetSquare can be reached in 5 moves from startSquare,
     *         false otherwise;
     **/
    ...
    private boolean isReachableInNMoves(int[] startSquare, int[] targetSquare, int n) {
        ...
        // Vérifier si la case cible est sur le même étage que la case de départ et à
        // une distance de dist déplacements horizontaux
        if (startY == targetY && deltaX == n)
            return true;

        // Vérifier si la case cible est accessible en utilisant au plus dist
        // déplacements alternés entre horizontal et vertical
    ...
    private ArrayList<int[]> getAllSquaresReachableWithNMoves(int[] startSquare, int n) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                if (isReachableInNMoves(startSquare, newSquare, 5))
